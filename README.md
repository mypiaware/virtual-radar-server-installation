...
This script has been confirmed to work with VRS version 2.4.4 (the latest official release) on Raspberry Pi OS Buster (32-bit -- Desktop & Lite) and Debian 10.  At this time, only these two operating systems are officially supported.  [(Please read the important note below regarding other operating systems!)](https://github.com/mypiaware/virtual-radar-server-installation/blob/master/README.md#mono-issue)  Note that "Raspberry Pi OS" was recently known as "Raspbian".
...
This script will install VRS on most popular Linux systems.  However, a number of the newer Debian-based operating systems (Ubuntu, Linux Mint, etc.) will install Mono with a version of 6.4 or higher.

The latest versions of Mono (versions greater or equal to 6.4) on some Debian-based operating systems appear to have an issue with the stable version of VRS (ver 2.4.4).  The issue is the plane icons and the altitude stalks will not appear on the VRS webpage.
